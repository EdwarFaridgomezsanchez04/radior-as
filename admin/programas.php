...
if (isset($_POST['action'])) {
    switch ($_POST['action']) {
        case 'toggle_status':
            $id = (int)$_POST['id'];
            $stmt = $con->prepare("UPDATE programas SET activo = NOT activo WHERE id = ?");
            $stmt->execute([$id]);
            break;
            
        case 'toggle_featured':
            $id = (int)$_POST['id'];
            $stmt = $con->prepare("UPDATE programas SET destacado = NOT destacado WHERE id = ?");
            $stmt->execute([$id]);
            break;
        ...
        case 'delete':
            ...
            $stmt = $con->prepare("SELECT imagen FROM programas WHERE id = ?");
            $stmt->execute([$id]);
            $programa = $stmt->fetch(PDO::FETCH_ASSOC);
            
            // Eliminar imagen física si existe
            if ($programa['imagen'] && file_exists(__DIR__ . '/../uploads/programas/images/' . $programa['imagen'])) {
                unlink(__DIR__ . '/../uploads/programas/images/' . $programa['imagen']);
            }
            
            // Eliminar de la base de datos
            $stmt = $con->prepare("DELETE FROM programas WHERE id = ?");
            $stmt->execute([$id]);
            break;
    }
...
if (!empty($search)) {
    $where_conditions[] = "(nombre_es LIKE ? OR nombre_gl LIKE ? OR descripcion_es LIKE ?)";
    $search_param = "%$search%";
    $params[] = $search_param;
    $params[] = $search_param;
    $params[] = $search_param;
...
$count_sql = "SELECT COUNT(*) FROM programas $where_clause";
...
// Obtener programas
$sql = "SELECT * FROM programas $where_clause ORDER BY fecha_creacion DESC LIMIT ? OFFSET ?";
$params[] = $limit;
$params[] = $offset;
...
<head>
    ...
    <title>Gestión de Programas - RadioRías Admin</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-black text-white">Gestión de Programas</h1>
                        <p class="text-gray-100">Panel de Administración</p>
                    </div>
                </div>
                
                <div class="flex items-center space-x-4">
                    <a href="programa_form.php" class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-lg font-bold transition-all duration-300 flex items-center space-x-2">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <form method="GET" class="flex flex-col md:flex-row gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="programas.php" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg font-bold transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div class="bg-white rounded-lg p-6 shadow-md border-l-4 border-blue-500">
                <div class="flex items-center">
                    <div class="flex-1">
                        <p class="text-blue-600 text-sm font-medium">Total Programas</p>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-bold text-gray-900">
                    Programas (<?php echo $total_records; ?> resultados)
                </h2>
            </div>
            ...
            <?php if (empty($programas)): ?>
                <div class="p-8 text-center">
                    ...
                    <p class="text-gray-500 text-lg">No se encontraron programas</p>
                    <a href="programa_form.php" class="inline-block mt-4 bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg font-bold transition-colors">
                        <i class="fas fa-plus mr-2"></i>Crear primer programa
                    </a>
                </div>
            <?php else: ?>
                                ...
                                <tr class="hover:bg-gray-50">
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-12 w-12">
                                                <?php if ($programa['imagen']): ?>
                                                    <img class="h-12 w-12 rounded-lg object-cover" 
                                                         src="../uploads/programas/images/<?php echo htmlspecialchars($programa['imagen']); ?>" 
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <div class="text-xs text-gray-400">
                                                    <?php echo htmlspecialchars(substr($programa['descripcion_es'], 0, 60)) . '...'; ?>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900"><?php echo htmlspecialchars($programa['horario']); ?></div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900"><?php echo htmlspecialchars($programa['dias']); ?></div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex flex-col space-y-1">
                                            <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full <?php echo $programa['activo'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'; ?>">
                                                <?php echo $programa['activo'] ? 'Activo' : 'Inactivo'; ?>
                                            </span>
                                            <?php if ($programa['destacado']): ?>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            <a href="programa_form.php?id=<?php echo $programa['id']; ?>" 
                                               class="text-blue-600 hover:text-blue-900 transition-colors" title="Editar">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            
                                            <form method="POST" class="inline" onsubmit="return confirm('¿Cambiar estado del programa?')">
                                                ...
                                                <button type="submit" class="text-green-600 hover:text-green-900 transition-colors" title="Cambiar estado">
                                                    <i class="fas fa-toggle-<?php echo $programa['activo'] ? 'on' : 'off'; ?>"></i>
                                                </button>
                                            </form>
                                            ...
                                            <form method="POST" class="inline" onsubmit="return confirm('¿Cambiar destacado?')">
                                                ...
                                                <input type="hidden" name="id" value="<?php echo $programa['id']; ?>">
                                                <button type="submit" class="text-yellow-600 hover:text-yellow-900 transition-colors" title="Destacar/Quitar destacado">
                                                    <i class="fas fa-star<?php echo $programa['destacado'] ? '' : '-o'; ?>"></i>
                                                </button>
                                            </form>
                                            
                                            <form method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de eliminar este programa? Esta acción no se puede deshacer.')">
