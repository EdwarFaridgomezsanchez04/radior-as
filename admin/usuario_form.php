...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $usuario_id_nuevo = $editing ? $id : $con->lastInsertId();
            
            if ($editing) {
                // Actualización
                $detalles_cambio = [];
                if (!empty($password)) $detalles_cambio['password_changed'] = true;
                if ($usuario['username'] !== $username) $detalles_cambio['username'] = ['old' => $usuario['username'], 'new' => $username];
                if ($usuario['email'] !== $email) $detalles_cambio['email'] = ['old' => $usuario['email'], 'new' => $email];
                if ($usuario['rol'] !== $rol) $detalles_cambio['rol'] = ['old' => $usuario['rol'], 'new' => $rol];
                if ($usuario['activo'] != $activo) $detalles_cambio['activo'] = ['old' => $usuario['activo'], 'new' => $activo];
                
                $historial->registrar('update', 'usuario', 'Actualizó usuario', [
                    'usuario_id' => $_SESSION['user_id'],
                    'usuario_nombre' => $_SESSION['username'],
                    'entidad_id' => $usuario_id_nuevo,
                    'entidad_nombre' => $username,
                    'detalles' => $detalles_cambio
                ]);
            } else {
                // Creación
                $historial->registrar('create', 'usuario', 'Creó nuevo usuario', [
                    'usuario_id' => $_SESSION['user_id'],
                    'usuario_nombre' => $_SESSION['username'],
                    'entidad_id' => $usuario_id_nuevo,
                    'entidad_nombre' => $username,
